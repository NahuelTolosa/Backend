...
public class MensajesController {
    ...
    public Mensaje getMensajeById(@PathVariable Long id) throws FirstApplicationException {
        logger.info("GET obtener mensaje por el id");

        if(id == 0) {
            throw new FirstApplicationException("El identificador del mensaje debe ser mayor a 0");
        }
        var msjFiltered = dataMensajes().stream()
